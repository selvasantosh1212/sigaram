...
import { getAllAttemptsForWeek, getAttempt, getAttemptAnswers } from "@/lib/attempts";
import { getTotalDays, getTotalWeeks, getWeekDayRange } from "@/lib/cycles";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Week {weekNumber} Cycle Test — Results</h1>
        <Link href="/weekly" className="text-sm text-zinc-600 hover:underline">
          All weeks
        </Link>
      </div>
      ...
      {attemptHistory.length > 1 && (
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-zinc-700">
            Attempt history for Week {weekNumber} ({attemptHistory.length} attempts)
          </h2>
          <div className="mt-2 space-y-1">
            {attemptHistory.map((a) => (
              <Link
                key={a.id}
                href={`/weekly/results/${a.id}`}
