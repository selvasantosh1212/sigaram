...
async function queryScoreTrend(attemptKind: "daily" | "full_mock"): Promise<ScoreTrendPoint[]> {
  const db = await getDb();
  const rs = await db.execute({
    sql: `SELECT id, day_number, session_label, submitted_at, score_percent, duration_seconds
       FROM attempts WHERE submitted_at IS NOT NULL AND attempt_kind = ? ORDER BY submitted_at ASC`,
    args: [attemptKind],
  });
  const rows = rowsToObjects(rs) as Array<{
    id: number;
...
export async function getDailyScoreTrend(): Promise<ScoreTrendPoint[]> {
  return queryScoreTrend("daily");
}

export async function getFullMockScoreTrend(): Promise<ScoreTrendPoint[]> {
  return queryScoreTrend("full_mock");
}

export type PartAccuracy = { part: "A" | "B" | "C"; correct: number; total: number; accuracyPercent: number };

export async function getPartAccuracy(): Promise<PartAccuracy[]> {
  const db = await getDb();
  const rs = await db.execute(
  ...
  const rows = rowsToObjects(rs) as Array<{ part: "A" | "B" | "C"; correct: number; total: number }>;
...
export async function getUnitAccuracy(): Promise<UnitAccuracy[]> {
  const db = await getDb();
  const rs = await db.execute(
  ...
  const rows = rowsToObjects(rs) as Array<{ unit_id: string; correct: number; total: number }>;
...
async function queryTopicAccuracyRows(): Promise<TopicAccuracy[]> {
  const db = await getDb();
  const rs = await db.execute(
  ...
  const rows = rowsToObjects(rs) as Array<{ topic_id: string; correct: number; total: number }>;
...
export async function getTopicAccuracy(): Promise<Map<string, TopicAccuracy>> {
  return new Map((await queryTopicAccuracyRows()).map((r) => [r.topicId, r]));
}
...
export async function getTopicLevelStats(): Promise<TopicLevelStat[]> {
  return (await queryTopicAccuracyRows())
    .filter((t) => t.total > 0)
...
export async function getDifficultyAccuracy(): Promise<DifficultyAccuracy[]> {
  const db = await getDb();
  const rs = await db.execute(
  ...
  const rows = rowsToObjects(rs) as Array<{ difficulty: "easy" | "medium" | "hard"; correct: number; total: number }>;
...
export async function getTimeOfDayDistribution(): Promise<TimeOfDayBucket[]> {
  const db = await getDb();
  const rs = await db.execute(`SELECT submitted_at FROM attempts WHERE submitted_at IS NOT NULL`);
  const rows = rowsToObjects(rs) as Array<{ submitted_at: string }>;
...
export async function getCompletedDaysCount(): Promise<number> {
  ...
  const row = rowsToObjects(rs)[0] as { c: number };
  return row.c;
}

export async function getAverageDuration(): Promise<number> {
  const db = await getDb();
  const rs = await db.execute("SELECT AVG(duration_seconds) as avg FROM attempts WHERE duration_seconds IS NOT NULL");
  const row = rowsToObjects(rs)[0] as { avg: number | null };
...
export async function getSourceTagAccuracy(): Promise<SourceTagAccuracy[]> {
  const db = await getDb();
  const rs = await db.execute(
  ...
  const rows = rowsToObjects(rs) as Array<{ source_tag: string; correct: number; total: number }>;
...
export async function getStreaks(): Promise<StreakInfo> {
  const db = await getDb();
  const rs = await db.execute(
    `SELECT mock_submitted_at FROM day_progress WHERE mock_submitted_at IS NOT NULL ORDER BY mock_submitted_at ASC`
  );
  const rows = rowsToObjects(rs) as Array<{ mock_submitted_at: string }>;
...
export async function getDaysSinceLastActivity(): Promise<number | null> {
  const db = await getDb();
  const rs = await db.execute(`SELECT MAX(submitted_at) as last FROM attempts WHERE submitted_at IS NOT NULL`);
  const row = rowsToObjects(rs)[0] as { last: string | null };
